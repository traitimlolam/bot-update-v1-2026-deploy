...
- **AC4**: Gửi số điện thoại được xác nhận **chính xác hợp lệ** qua tin nhắn Messenger (đúng độ dài + đúng đầu số) → Sheet có dòng mới đúng cột A, B, C, F và cột E = `"Tin nhắn"` (cột D thêm nếu nhân viên có ghi tay; cột G để trống; cột H luôn để trống, không xử lý) → khách nhận 1 tin `AI_PHONE_CONFIRMED`.
